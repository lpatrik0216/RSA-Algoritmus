...
public class PrimeCheck {
    ...
    //Returns the S value
    private static long getSValue(BigInteger prime){
        long S=0;
    ...
    //Returns the d value
    private static BigInteger getDValue(BigInteger prime){
        prime=prime.subtract(BigInteger.ONE);
        while(prime.remainder(BigInteger.TWO).equals(BigInteger.ZERO)){
            prime = prime.divide(BigInteger.TWO);
        }
    ...
    //Generates a LIST of bases that will be used during the Miller-Rabin test. It generates n random numbers (n being the number of required rounds)
    private static List<Integer> generateBases(int numberOfRounds){
    ...
    public static boolean isPrime(BigInteger prime, int numberOfRounds){ //Runs a Miller-Rabin test, for the given rounds.
        long S=getSValue(prime);
        BigInteger d = getDValue(prime);

        //An error, if there are more requested rounds than possible bases.
        ...
        for (Integer current : roundBasesList){//current will be the base for the given round
            ...
            for (int i = 0; i < S; i++) { //i will be the number of round within the round
                ...
                if(i==0){ //Checks the first Miller-Rabin case
                ...
                else{ //Checks the second Miller-Rabin case
